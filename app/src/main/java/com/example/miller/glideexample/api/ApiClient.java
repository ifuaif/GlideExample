...
import com.example.miller.glideexample.BuildConfig;
...
//	https://api.nasa.gov/mars-photos/api/v1/rovers/curiosity/photos?earth_date=2015-6-3&api_key=DEMO_KEY
		...
		final OkHttpClient okHttpClient = new OkHttpClient.Builder()
				...
				.hostnameVerifier(new HostnameVerifier() {
					...
					public boolean verify(final String hostname, final SSLSession session) {
						return BASE_URL.contains(hostname);
					}
				})
				.build();
	...
	public static void getPhotos(final String date, final Callback<ModelSpacePhoto> callback) {
		sRetrofit.create(GetApiSpace.class).getAllInformationAboutDateFoto(date).enqueue(callback);
	}
